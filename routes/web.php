...
Route::group(
    ...
    Route::middleware(['auth:sanctum', 'verified'])->get('/dashboard', function () {
		...
		$track_Africa = Tracking::where('country_id', '=', 12)->count();
		$track_Bahrain = Tracking::where('country_id', '=', 13)->count();
		$track_Indonesia = Tracking::where('country_id', '=', 14)->count();
		$Track_All_weight = Tracking::select('weight')->sum('weight');
        $Emirates_weight = Tracking::select('weight')->where('country_id', '=', 3)->sum('weight');
        ...
        return view('dashboard', compact('userData', 'Tracking', 'track_st', 'noty', 'clients', 'users', 'track_cartons_number', 'track_pieces_number',
            ...
            'track_Egypt', 'track_QATAR', 'track_SANMARTIN','Tracking_Archives','track_USA','track_OMAN'
										 ,'track_Namibia','Track_All_weight','Emirates_weight','track_Africa','track_Bahrain','track_Indonesia'));
    })->name('dashboard');
...
//==============================status track============================
 ...
 Route::get('/tracking-invoice/{id}', [InvoicesController::class, 'AddInvoices'])->name('add.invoice');
...
Route::group(
    ...
    Route::prefix('client')->group(function () {
        ...
        Route::get('/beforelogin', [ClientController::class, 'ClientbeforeLogin'])->name('client_before_login');
